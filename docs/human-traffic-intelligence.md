...
**1. Footfall** (вес 25%)
```
Срабатывает: annualEntries >= 1,000,000
Формула: (log(entries) - log(1M)) / (log(max) - log(1M))
  где max = максимум по всем станциям (98M Liverpool Street)
```
Чем больше пассажиров, тем выше strength, но по log-шкале — разница между 1M и 10M важнее, чем между 80M и 90M.
...
**2. Brand Gap** (вес 25%)
```
Срабатывает: бренд = 0 точек в 800м И хотя бы 1 QSR конкурент есть
Формула: 0.6 + min(qsrCount800m / 10, 1) × 0.2
```
Базовый strength 0.6 за сам факт отсутствия бренда. Бонус до 0.2 за количество конкурентов — чем больше конкурентов без нашего бренда, тем более "подтверждён" спрос в этой точке.
...
Три приоритетных пути — используется первый подходящий:

*Путь A: Бизнес-район (50K+ workers, данные о зарплатах)*
```
Формула (premium brand, salary >= £30K):
  strength = 0.7 + min((salary - 30000) / 5000, 0.3) × 0.3
Формула (value brand, salary < £27K):
  strength = 0.6 + min((27000 - salary) / 4000, 0.3) × 0.3
Neutral brand: strength = 0.5
Mismatch: strength = 0.3
```

*Путь B: Local income decile (1.5km, >95% станций)*
```
Premium brand (decile >= 6): strength = 0.7 + (decile - 6) × 0.1
  decile 6 → 0.7, decile 8 → 0.9, decile 10 → 1.1 (capped at 1.0)
Value brand (decile <= 5): strength = 0.6 + (5 - decile) × 0.1
  decile 5 → 0.6, decile 3 → 0.8, decile 1 → 1.0
Neutral brand (decile >= 3): strength = 0.5
```

*Путь C: Region fallback (только если нет local data)*
Та же формула что путь B, но с regional medianIncomeDecile.

**4. Low Density** (вес 15%)
```
Срабатывает: qsrCount800m / avgQsr < 0.75 (меньше 75% от среднего)
Формула: (1 - ratio / 0.75) × 0.8
  где ratio = qsrCount800m / avgQsr
```
Линейная шкала: чем меньше QSR относительно среднего, тем выше strength.
...
**Пример: "Density 18" означает:** станция имеет QSR density ~67% от среднего. `(1 - 0.67/0.75) × 0.8 ≈ 0.09`, в composite score это `0.09 × 15% weight ≈ 1.3 пункта`. Финальное число в UI = `round(strength × weight / totalFiredWeight × 100)`.

**5. Pedestrian** (вес 8%)
```
Срабатывает: busStopCount800m >= 30
Формула: min((busStops - 30) / 70, 1) × 0.6 + 0.3
```
Линейная шкала от 30 до 100 bus stops.
...
```
Срабатывает: AADF >= 50,000 авто/день И driveThruCount < 2
Формула: min((aadf - 50000) / 100000, 1) × 0.4 + 0.5
```
Линейная шкала от 50K до 150K авто/день.

| AADF | Strength | Как читать |
|------|----------|-----------|
| 50K | 0.50 | Порог — оживлённая дорога |
| 75K | 0.60 | Магистраль |
...
**7. Workforce** (вес 5%)
```
Срабатывает: workplacePop1500m >= 10,000
Формула: min((log(wp) - log(10K)) / (log(100K) - log(10K)), 1) × 0.6 + 0.3
```
Log-шкала (распределение workforce сильно скошено — несколько точек с 100K+, большинство 10-30K).

| Workers | Strength | Как читать |
|---------|----------|-----------|
| 10K | 0.30 | Небольшой деловой район |
| 20K | 0.48 | Средний деловой район |
